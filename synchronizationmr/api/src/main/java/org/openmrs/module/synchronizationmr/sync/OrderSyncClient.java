...
public class OrderSyncClient {
	...
	private final OrderSyncService records;
	
	private final OrderReceiveService receiver;
	 ...
	 */
	public static OrderSyncClient forLocalPosta(String endpoint, String masterServerId, String username, String password)
	        throws IOException {
		...
		return new OrderSyncClient(Context.getService(LocalNodeService.class), Context.getService(OrderSyncService.class),
		        Context.getService(OrderReceiveService.class), new PatientHttpsTransport(endpoint, username, password),
		        masterServerId);
	}
	
	public OrderSyncClient(LocalNodeService node, OrderSyncService records, OrderReceiveService receiver,
	...
	public int[] synchronizeOnce() throws IOException {
		...
		long highest = records.getHighestOrderSequence(local);
		...
		while (acknowledged < highest && sent < 100) {
			checkInterrupted();
			List<OrderSyncEvent> page = records.getOrderEventsAfter(local, acknowledged, 1);
		...
		while (true) {
			...
			for (JsonNode entry : origins) {
				...
				long after = receiver.getConfirmedOrderSequence(origin);
				...
				for (int count = 0; count < 100; count++) {
					...
					long confirmed = receiver.receiveOrder(event.toString());
	...
	private static void checkOrigin(JsonNode value, String origin) throws IOException {
		require(origin.equals(value.path("originServerId").asText()) && "ORDER".equals(value.path("entityType").asText()),
